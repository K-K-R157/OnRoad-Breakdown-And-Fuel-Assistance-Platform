...
fuelRequestSchema.pre('save', function(next) {
  if (this.isModified('quantity') || this.isModified('pricePerLiter') || this.isModified('deliveryCharges')) {
      ...
      case 'delivered':
        if (!this.deliveredAt) {
          ...
          if (this.dispatchedAt) {
            this.deliveryDuration = Math.floor((now - this.dispatchedAt) / 60000);
          }
        }
        break;
      case 'cancelled':
...
fuelRequestSchema.methods.updateStatus = async function(newStatus, additionalData = {}) {
...
fuelRequestSchema.methods.processPayment = async function(method, transactionId) {
