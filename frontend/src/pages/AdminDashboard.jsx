      ...
      <div className="max-w-6xl mx-auto">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === "charging" && <ChargingStationsTab token={token} />}
...
function ChargingStationsTab({ token }) {
  const [stations, setStations] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [expandedId, setExpandedId] = useState(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await adminAPI.getPendingChargingStations(token);
        setStations(res.data || []);
      } catch (err) {
        setError(err.message);
      } finally {
    ...
    <div className="space-y-6">
      ...
      {stations.length === 0 ? (
        <div className="text-center py-16">
          <CheckCircle2 className="w-12 h-12 text-emerald-400/50 mx-auto mb-3" />
          <p className="text-slate-400">
            All charging stations have been reviewed!
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                    ...
                    <div className="px-5 pb-5 border-t border-white/5 pt-4 space-y-3">
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <Detail
                          ...
                          value={s.mobileChargingAvailable ? "Yes" : "No"}
                        />
                        <Detail
                          ...
                          <div className="flex flex-wrap gap-2">
                            {s.chargingTypes.map((ct, i) => (
                              ...
                              >
                                ...
                                {ct.pricePerKwh}/kWh
                                <span
                                  className={`text-[9px] ml-1 ${ct.available ? "text-emerald-400" : "text-red-400"}`}
                                >
                                  {ct.available ? "avail" : "out"}
                                </span>
                              </span>
                            ))}
                          </div>
